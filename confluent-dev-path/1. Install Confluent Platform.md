...
## Set Environment Variable di systemd

Buat direktori baru untuk service:

```
sudo mkdir /etc/systemd/system/confluent-server.service.d
```

Buat file override systemd untuk service confluent-server untuk meningkatkan memori heap menjadi 4 GB.
...
Jalankan confluent-server dengan command berikut:
...
![image](https://github.com/user-attachments/assets/cd1cad98-5683-452b-b2dd-4f9459c6766e)

## Log dalam Kafka

![image](https://github.com/user-attachments/assets/f932b24d-849e-431e-8311-59e2bff95fcc)

Satu hal yang sering membingungkan dalam Kafka adalah penggunaan kata "log" dalam konteks yang berbeda. Bagi banyak profesional, "log" berarti log aplikasi (seperti pesan INFO atau DEBUG), yang disimpan di direktori seperti /var/log/kafka. Kafka menggunakan log4j untuk log aplikasi ini.

Namun, dalam konteks Kafka, "log" merujuk pada partisi topik yang disimpan dalam struktur data log komit. Perbedaan ini penting karena banyak properti konfigurasi Kafka menggunakan kata "log" untuk merujuk pada partisi topik, bukan log aplikasi.

## Eksplorasi File Properties dan CLI

Berikut adalah contoh-contoh properties yang ada di file server.properties:
...
Berikut adalah alat CLI yang disertakan dalam confluent platform:
