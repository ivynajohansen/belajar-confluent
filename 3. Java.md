...
2. Buat file 'NotifConsumer.java" yang isinya:
...
4. Buat file untuk logging Kafka bernama `log4j.properties`:
...
2. Untuk membuat Kafka Admin Client di Java yang menghapus topic `notifications` sebelumnya dan membuat `notifications-new`, buat file baru `Admin.java` yang isinya:
...
```
[Unit]
Description=Kafka Notifications Service
After=network.target

[Service]
User=root
ExecStart=/usr/bin/java -cp /root/confluent-ivy/kafka-notifications.jar NotifProducer
Restart=always
...
Buat file yang sama untuk Consumer dan Admin. Ubah `NotifProducer` menjadi class yang lain.
